...
from app_figures import values, statepredictorGraph, stateDeseased, stateRecovered, stateActive, statePie, districtPie, rate
...
PLOTLY_LOGO1 = "../assets/paclogo.jpg"
import dash_table
...
body = html.Div([
    ...
    , dbc.Row([
        dbc.Col((html.Div(dbc.Alert("CASES IN KARNATAKA", color="dark")),
                 ...
                 html.Div([
            dcc.Graph(
                figure= rate('Recovery Rate', 50, 5), style={"height": "20rem"}
            )
        ]
        ...
        , dbc.Col((
            html.Div(
                ...
                dcc.Tabs(id='tabs-example', value='tab-1', children=[
                    ...
                    dcc.Tab(label='About', value='tab-5', style=tab_style, selected_style=tab_selected_style)
...
def render_content(tab):
    if tab == 'tab-1':
        return html.Div([
            dcc.Graph(
                figure=statepredictorGraph(), style={"height": "36rem"}
            )
        ])
    elif tab == 'tab-2':
        return html.Div([
            dcc.Graph(
                figure=stateActive(), style={"height": "36rem"}
            )
        ])
    elif tab == 'tab-3':
        return html.Div([
            dcc.Graph(
                figure=stateRecovered(), style={"height": "36rem"}
            )
        ])
    elif tab == 'tab-4':
        return html.Div([
            ...
            dcc.Graph(
                figure=stateDeseased(), style={"height": "36rem"}
            )
        ])
        ...
        return html.Div([
            html.Div(dbc.Alert([
                                  html.H4("Preface"),
                                   html.P(
                                       "Public Affairs Centre (PAC) engages in action research focussing on Sustainable Development Goals"
        ...
        " Designing and Editing by: PEC")
                ...
                html.P(
                    ...
                    "The CODR COVID-19 DASHBOARD is based on data available in the public domain. While every care is taken to develop an accurate prediction model, the model is prone to complex, evolving, and heterogeneous epidemiology of the COVID-19 disease. The predictions are subjected to an error rate of 10% or less, acceptable under prediction norms in Artificial Intelligence and Machine Learning."),

                html.P(
                    "The website and its contents herein, including analysis, mapping, and predictions are subjected to Copyright 2020 PAC."),

                html.P(
                    "Suggested Citation - Public Affairs Centre, Centre for Open Data Research. CODR COVID-19 DASHBOARD [Internet]. http://codrindia.in/. [cited Date]. Available from: http://codrindia.in")],
                color="active"))
        ])

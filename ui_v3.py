...
body = html.Div([
    ...
    , dbc.Row([
        dbc.Col((html.Div(dbc.Alert("CASES IN KARNATAKA", color="light")),
                 ...
                 html.Div(dbc.Alert("Districts", color="light")),
                 html.Div(
                     dash_table.DataTable(
                         data=df.to_dict('records'),
                         columns=[{'id': c, 'name': c} for c in df.columns],
                         fixed_rows={'headers': True},
                         style_table={'height': 260},  # defaults to 500,
...
def render_content(tab):
    if tab == 'tab-1':
        return html.Div([
            dcc.Graph(
                figure=statepredictorGraph(), style={"height": "37rem"}
            )
        ])
    elif tab == 'tab-2':
        return html.Div([
            dcc.Graph(
                figure=stateActive(), style={"height": "37rem"}
            )
        ])
    elif tab == 'tab-3':
        return html.Div([
            dcc.Graph(
                figure=stateRecovered(), style={"height": "37rem"}
            )
        ])
    elif tab == 'tab-4':
        return html.Div([
            ...
            dcc.Graph(
                figure=stateDeseased(), style={"height": "37rem"}
            )
        ])
